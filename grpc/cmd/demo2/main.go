...
type StreamService struct{
	pb.UnimplementedBothStreamServer
}
// Conversations 实现Conversations方法
func (s *StreamService) Conversations(srv pb.BothStream_ConversationsServer) error {
	n := 1
	for {
		req, err := srv.Recv()
		if err == io.EOF {
...
func main() {
	...
	// 在gRPC服务器注册我们的服务
	pb.RegisterBothStreamServer(grpcServer, &StreamService{})
